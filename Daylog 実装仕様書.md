...
適用済みのスキーマバージョンを `PRAGMA user_version` に持たせ、起動時に
未適用のものだけを順に適用する。各バージョンの処理と `user_version` の
更新は同一トランザクションで行う。

```text
v1  バージョン管理導入時点のスキーマ一式
    （それ以前に起動時へ直接書かれていた移行処理も取り込む）
v2  検索インデックスをbigram方式へ作り直す
```

バージョン管理の導入前に作られたデータベースは `user_version` が既定値の
0 なので、「v1 未適用」として自然に扱える。各バージョンの処理は冪等に書く。

`user_version` が実行中のバイナリの最新版より大きい場合は、古いバイナリで
新しいデータを壊さないよう起動を中止する。

未適用の移行がある場合は、適用前に `backups/pre-migration_<日時>.db` へ
データベースを退避する（3世代を保持）。
...
```text
entity_type   UNINDEXED  task / entry / note_card / review / ai_summary
entity_id     UNINDEXED
day_id        UNINDEXED
day_date      UNINDEXED  期間での絞り込みをJOINなしで行うため非正規化
content       UNINDEXED  抜粋の生成に使う原文
tokens                   bigram化した検索対象（tokenize='unicode61'）
```
...
`unicode61` は日本語を分かち書きしないため、格納時と検索時の両方で本文を
隣接2文字のトークン列（bigram）へ変換してから索引する。例：`会議室` →
`会議 議室`。検索語も同じ変換を通してフレーズ検索にすることで、原文での
部分文字列一致と同じ結果になる。

SQLite 3.34以降の `trigram` トークナイザは採用しない。3文字未満のクエリを
原理的にマッチできず、「会議」「体調」のような2文字語が支配的な日本語の
日誌検索では使えないため。

検索語の記法：空白区切りはAND、`OR` でOR、先頭の `-` で除外、`"…"` で
フレーズ。FTS5の特殊文字はトークン化の段階で区切りとして落ちるため、
ユーザー入力が原因でMATCHの構文エラーになることはない。

並び順は検索語があるとき `bm25()` の関連度順、ないとき日付の新しい順。
結果は `limit` / `offset` でページングする。

抜粋は FTS5 の `snippet()` を使わず、`content`（原文）から検索語の周辺を
切り出して生成する。`snippet()` は bigram 化した列に対して働くため、
「会議 議室」のような壊れた文字列になるためである。
...
AIの実行記録も同じファイルに出す（当初はAI用に別ファイルを分ける想定だったが、
単一プロセスで出力量も少なく、障害時に時系列で追えるほうが有用なため一本化した）。

個人のジャーナル本文をログへ書き込まない。記録してよいのは操作名・件数・
日付キー・経過時間・エラー文字列など、内容を復元できない情報に限る。

起動時にファイルサイズが5MBを超えていれば `logs/daylog.log.1` へ退避する
（1世代のみ）。
...
```text
Weekly Review       ふりかえり画面（週）
Monthly Review      ふりかえり画面（月）
去年の今日          ふりかえり画面の下部
今日の一行一覧      ふりかえり画面のAI一行まとめ
Markdown Export     日単位・メモ単位
添付ファイル
画像貼り付け
テーマ              7種
ダークモード        テーマ「夜」

...
import type { AiStatus, DayData, Entry, Review, SaveStatus, Settings, Tag } from "../types";
...
interface Props { day: DayData; tags: Tag[]; settings: Settings; onDay: Dispatch<SetStateAction<DayData | null>>; onOpenDate: (date: string) => Promise<void>; onError: (message: string) => void; }
export interface TodayPageHandle { flush: () => Promise<void>; }
export const TodayPage = forwardRef<TodayPageHandle, Props>(function TodayPage({ day, tags, settings, onDay, onOpenDate, onError }, ref) {
    ...
    <div className={`page-inner ${settings.layout === "two" ? "wide" : settings.layout === "three" ? "three-wide" : ""}`}>
      ...
      <div className={`today-grid ${settings.layout}`}>
        ...
        <div className="column primary-column">
          <TaskSection tasks={day.tasks} availableTags={tags} dayDate={day.dayDate} disabled={day.isClosed} onError={onError} onTasksChange={(tasks) => updateCurrentDay((current) => ({ ...current, tasks }))} onAdd={(title) => required(async () => { const task = await api.createTask(day.dayDate, title); updateCurrentDay((current) => ({ ...current, tasks: [...current.tasks, task] })); })} onToggle={(task) => guarded(async () => { const next = await api.updateTask({ ...task, isCompleted: !task.isCompleted }); updateCurrentDay((current) => ({ ...current, tasks: current.tasks.map((item) => item.id === next.id ? next : item) })); })} onUpdate={async (task) => { const next = await api.updateTask(task); updateCurrentDay((current) => ({ ...current, tasks: current.tasks.map((item) => item.id === next.id ? next : item) })); return next; }} onSetTags={async (id, ids) => { const selected = await api.setTaskTags(id, ids); updateCurrentDay((current) => ({ ...current, tasks: current.tasks.map((task) => task.id === id ? { ...task, tags: selected } : task) })); return selected; }} onDelete={async (id) => { await api.deleteTask(id); updateCurrentDay((current) => ({ ...current, tasks: current.tasks.filter((task) => task.id !== id) })); }} onReorder={(orderedIds) => api.reorderTasks(day.dayDate, orderedIds)}/>
          <TimelineSection entries={day.entries} availableTags={tags} dayDate={day.dayDate} disabled={day.isClosed} onError={onError} onAdd={(body, icon) => required(async () => { const entry = await api.createEntry(day.dayDate, body, icon); updateCurrentDay((current) => ({ ...current, entries: [...current.entries, entry] })); })} onUpdate={saveEntry} onIcon={(entry, icon) => required(async () => { await saveEntry({ ...entry, icon }, day.dayDate); })} onSetTags={async (id, ids) => { const selected = await api.setEntryTags(id, ids); updateCurrentDay((current) => ({ ...current, entries: current.entries.map((entry) => entry.id === id ? { ...entry, tags: selected } : entry) })); return selected; }} onDelete={async (id) => { await api.deleteEntry(id); updateCurrentDay((current) => ({ ...current, entries: current.entries.filter((entry) => entry.id !== id) })); }}/>
        </div>
        <div className="column secondary-column">
          <DailyNoteSection ref={noteSection} notes={day.notes} availableTags={tags} disabled={day.isClosed} onError={onError} onCardsChange={(notes) => updateCurrentDay((current) => ({ ...current, notes }))} onCreate={async () => { const card = await api.createNoteCard(day.dayDate); updateCurrentDay((current) => ({ ...current, notes: [...current.notes, card] })); return card; }} onSave={async (card) => { const saved = await api.updateNoteCard(card); updateCurrentDay((current) => ({ ...current, notes: current.notes.map((note) => note.id === saved.id ? saved : note) })); return saved; }} onSetTags={async (id, ids) => { const selected = await api.setNoteCardTags(id, ids); updateCurrentDay((current) => ({ ...current, notes: current.notes.map((note) => note.id === id ? { ...note, tags: selected } : note) })); return selected; }} onDelete={async (id) => { await api.deleteNoteCard(id); updateCurrentDay((current) => ({ ...current, notes: current.notes.filter((note) => note.id !== id) })); }} onReorder={async (orderedIds) => { const notes = await api.reorderNoteCards(day.dayDate, orderedIds); updateCurrentDay((current) => ({ ...current, notes })); return notes; }}/>

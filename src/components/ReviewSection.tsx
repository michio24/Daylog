  ...
  return <section className="card review-section" id="daily-review">
    ...
    {open && <div className="review-body">
      ...
      {(settings.aiEnabled || aiSummary) && <div className="ai-area">
        ...
        {aiSummary && <div className="ai-summary"><h3>AIによる振り返りとアドバイス</h3><p>{aiSummary.summary}</p><h4>成果</h4><ul>{aiSummary.achievements.map((a) => <li key={a}>{a}</li>)}</ul><h4>明日試せる小さな一歩</h4>{aiSummary.tomorrowCandidates.map((c) => <div className="candidate" key={c}><span>{c}</span><button onClick={() => onCandidate(c)}>＋ 明日のタスクへ</button></div>)}<blockquote>{aiSummary.oneLine}</blockquote></div>}
      </div>}

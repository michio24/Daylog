...
interface Props { review: Review; disabled: boolean; settings: Settings; saveStatus: "saved" | "saving" | "error"; canRunAi: boolean; aiStatus: AiStatus; aiSummary?: AiSummary | null; onChange: (review: Review) => void; onBlur: () => void; onRunAi: () => void; onCancelAi: () => void; onCandidate: (title: string) => void; onClose: () => void; }
export function ReviewSection({ review, disabled, settings, saveStatus, canRunAi, aiStatus, aiSummary, onChange, onBlur, onRunAi, onCancelAi, onCandidate, onClose }: Props) {
  ...
  return <section className="card review-section" id="daily-review">
    ...
    {open && <div className="review-body">
      ...
      <div className="close-row"><span>{disabled ? "完了済み" : "書き終えたら、今日を閉じる"}</span><button className="primary" disabled={disabled} onClick={onClose}>{disabled ? "完了しました" : "今日を完了"}</button></div>
    </div>}
  </section>;
}
